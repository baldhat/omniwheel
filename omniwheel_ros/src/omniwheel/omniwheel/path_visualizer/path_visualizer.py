...
class PathVisualizer(Node):
    ...
    def register_mouse_button_event_handlers(self):
        self.mouse_button_event_handlers.append((pygame.MOUSEBUTTONUP, 2, lambda event: self.set_middle_mouse_down(False)))
        self.mouse_button_event_handlers.append((pygame.MOUSEBUTTONDOWN, 2, lambda event: self.set_middle_mouse_down(True)))
        self.mouse_button_event_handlers.append((pygame.MOUSEBUTTONDOWN, 1,
                                          lambda event: self.robot.add_waypoint(self.renderer.to_real_pos(event.pos),
                                                                                not pygame.key.get_pressed()[
                                                                                    pygame.K_LSHIFT])))
